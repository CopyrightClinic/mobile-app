...
final class AnalyticsInitializer {
  AnalyticsInitializer({
    ...
    required FacebookAppEvents meta,
  ...
  Future<void> initialize() async {
    await _firebaseAnalytics.setAnalyticsCollectionEnabled(
      Config.analyticsFirebaseEnabled,
    );
    if (!kReleaseMode) {
      debugPrint(
        '[analytics] Firebase Analytics collection enabled: ${Config.analyticsFirebaseEnabled}',
      );
    }
    TrackingStatus iosAttStatus = TrackingStatus.notDetermined;
    final needsAtt =
        Platform.isIOS &&
        (Config.analyticsMetaEnabled || Config.analyticsTikTokEnabled);
    if (needsAtt) {
    ...
    if (Config.analyticsMetaEnabled) {
      ...
      if (Platform.isIOS) {
        await _meta.setAdvertiserTracking(
          enabled: iosAttStatus == TrackingStatus.authorized,
        );
      } else {
        await _meta.setAdvertiserTracking(enabled: true);
      }
      await _meta.setAutoLogAppEventsEnabled(
        Config.analyticsMetaAutoLoggingEnabled,
      );
      if (!kReleaseMode) {
        debugPrint(
          '[analytics] Meta App Events SDK configured (autoLog=${Config.analyticsMetaAutoLoggingEnabled})',
        );
      }
    } else if (!kReleaseMode) {
      debugPrint('[analytics] Meta App Events SDK skipped (disabled in config)');
    }
    final tiktokIosReady =
        Platform.isIOS &&
        Config.tikTokIosAppleAppStoreId.isNotEmpty &&
        Config.tikTokIosTikTokAppId.isNotEmpty;
    final tiktokAndroidReady =
        Platform.isAndroid &&
        Config.tikTokAndroidAppId.isNotEmpty &&
        Config.tikTokAndroidSdkKey.isNotEmpty;
    if (Config.analyticsTikTokEnabled &&
        (tiktokIosReady || tiktokAndroidReady)) {
      final consentStatus =
          Platform.isIOS
              ? (iosAttStatus == TrackingStatus.authorized
                  ? 'granted'
                  : 'denied')
              : 'granted';
      await TikTokEventsSdk.initSdk(
        androidAppId: Config.tikTokAndroidAppId,
        tikTokAndroidId: Config.tikTokAndroidSdkKey,
        iosAppId: Platform.isIOS ? Config.tikTokIosAppleAppStoreId : '',
        tiktokIosId: Platform.isIOS ? Config.tikTokIosTikTokAppId : '',
        isDebugMode: Config.analyticsTikTokDebug,
        logLevel:
            Config.analyticsTikTokVerboseLogs
                ? TikTokLogLevel.debug
                : TikTokLogLevel.info,
        androidOptions: const TikTokAndroidOptions(disableAutoStart: true),
        iosOptions: TikTokIosOptions(
          accessToken:
              Config.tikTokIosAccessTokenForSdk.isEmpty
                  ? null
                  : Config.tikTokIosAccessTokenForSdk,
      ...
      if (!kReleaseMode) {
        debugPrint(
          '[analytics] TikTok Events SDK initialized and startTrack() completed',
        );
      }
    } else if (!kReleaseMode) {
      debugPrint(
        '[analytics] TikTok Events SDK skipped (disabled or platform credentials incomplete)',
      );
    }
    if (!kReleaseMode) {
